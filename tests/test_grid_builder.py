...
from airlock.grid_builder import (
    ...
    gridcells_from_geodataframe,
)
from airlock.models import GridCell
...
def test_gridcells_from_geodataframe():
    """
    Ensure GridCell models are created correctly from a GeoDataFrame.
    """
    df = pd.DataFrame({
        "X": [500000],
        "Y": [200000],
    ...
    gdf = build_grid_geodataframe(df)
    cells = gridcells_from_geodataframe(gdf, id_column="GridCode")

    assert len(cells) == 1

    cell = cells[0]
    assert isinstance(cell, GridCell)
    assert cell.id == "12345"
    assert cell.center_x == 500000
    assert cell.center_y == 200000
    assert cell.geometry.bounds == (499500, 199500, 500500, 200500)
